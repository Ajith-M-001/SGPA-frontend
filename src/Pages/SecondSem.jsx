...
const SecondSem = () => {
  ...
  const [internal9, setInternal9] = useState("");
  const [external9, setExternal9] = useState("");
  const [total9, setTotal9] = useState("0");
  const [credit9, setCredit9] = useState("0");
  const [grade9, setGrade9] = useState("0");

  const [internal9Error, setInternal9Error] = useState("");
  const [external9Error, setExternal9Error] = useState("");

  const handleSubjectChange9 = (e) => {
    const selectedSubject = e.target.value;

    const credit = e.target.selectedOptions[0].getAttribute("data-credit");

    setSubject9(selectedSubject);
    setCredit9(parseInt(credit));
  };

  const handleInternalChange9 = (e) => {
    const internalMark = Number.parseInt(e.target.value, 10);
    setInternal9(internalMark);
  };

  const handleExternalChange9 = (e) => {
    const externalMark = Number.parseInt(e.target.value, 10);
    setExternal9(externalMark);
  };

  useEffect(() => {
    const updateTotalAndgrade9 = () => {
      const internalMarks = parseFloat(internal9) || 0;
      const externalMarks = parseFloat(external9) || 0;
      const totalMarks = internalMarks + externalMarks;

      setTotal9(totalMarks);
  ...
  const handleSubmit = (e) => {
    ...
    // if (
    //   (!internal1Value || isNaN(internal1Value)) &&
    //   (!external1Value || isNaN(external1Value))
    // ) {
    ...
    //   if (internal1Value < 0 || internal1Value > 40) {
    //     setInternal1Error("Internal marks should be between 0 and 40.");
    //     hasErrors = true;
    //   }

    //   if (external1Value < 0 || external1Value > 60) {
    ...
    const internal9Value = parseInt(internal9);
    const external9Value = parseInt(external9);

    const isinternal9Valid =
      !isNaN(internal9Value) && internal9Value >= 0 && internal9Value <= 40;
    const isexternal9Valid =
      !isNaN(external9Value) && external9Value >= 0 && external9Value <= 60;

    if (!isinternal9Valid && !isexternal9Valid) {
      setInternal9Error("Please enter the marks.");
      setExternal9Error("Please enter the marks.");
      hasErrors = true;
    } else {
      setInternal9Error("");
      setExternal9Error("");
      ...
      if (!isinternal9Valid && isexternal9Valid) {
        setInternal9Error("Please enter the internal marks.");
        hasErrors = true;
      }

      if (isinternal9Valid && !isexternal9Valid) {
        setExternal9Error("Please enter the external marks.");
        hasErrors = true;
      }

      if (!isinternal9Valid) {
        setInternal9Error("Internal marks should be between 0 and 40.");
        hasErrors = true;
      }

      if (!isexternal9Valid) {
        setExternal9Error("External marks should be between 0 and 60.");
        hasErrors = true;
      }
    }
    ...
    subjectDropdowns.forEach((dropdown) => {
      ...
      if (!selectedSubject) {
        ...
        // Check subject selection rules
        switch (selectedSubject) {
          case "20MCA251":
            if (selectedSubjects.includes("20MCA255")) {
              errorDiv.textContent =
                "You cannot select both 20MCA251 and 20MCA255.";
              errorDiv.classList.remove("hidden");
            ...
            break;
          case "20MCA255":
            if (selectedSubjects.includes("20MCA251")) {
              errorDiv.textContent =
                "You cannot select both 20MCA251 and 20MCA255.";
              errorDiv.classList.remove("hidden");
              errorDiv.classList.add("text-red-500");
            ...
            break;
          case "20MCA262":
            if (selectedSubjects.includes("20MCA263")) {
              errorDiv.textContent =
                "You cannot select both 20MCA262 and 20MCA263.";
              errorDiv.classList.remove("hidden");
              errorDiv.classList.add("text-red-500");
              dropdown.addEventListener("focus", () => {
                errorDiv.classList.add("hidden");
            ...
            break;
          case "20MCA263":
            if (selectedSubjects.includes("20MCA262")) {
              errorDiv.textContent =
                "You cannot select both 20MCA262 and 20MCA263.";
              errorDiv.classList.remove("hidden");
              errorDiv.classList.add("text-red-500");
  ...
  useEffect(() => {
    ...
    const getErrorStateSetter = (inputId) => {
      const errorStateSetterMap = {
        ...
        internal9: setInternal9Error,
        external9: setExternal9Error,
      };
  ...
  //   // alert("working fine");
  //   let total = 0;
  //   const totalmarks = 900;

  //   let failCount = 0;
  //   const numberOfSubjects = 9; // Update this with the actual number of subjects
  ...
  //   try {
  //     const URL = "http://localhost:3000/api/2ndsem"; // Replace with your actual backend URL
  ...
  const sendCalculatedData = async (
    ...
    try {
      const URL = "https://sgpacalculatorbackend.onrender.com/api/2ndsem"; // Replace with your actual backend URL
      const response = await axios.post(`${URL}/add`, dataToSend);
      console.log("Data successfully sent to the backend:", response.data);
      const { message } = response.data;
      console.log(message);
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 sm:grid-cols-6 lg:grid-cols-6 gap-4 second_field">
                <div className="mb-6">
                  ...
                  >
                    ...
                    <option value="20MCA21" data-credit="3">
                      20MCA21 (DBMS)
                    </option>
                    <option value="20MCA22" data-credit="3">
                      20MCA22 (JAVA)
                    </option>
                    <option value="20MCA23" data-credit="4">
                      20MCA23 (WEB)
                    </option>
                    <option value="20MCA24" data-credit="4">
                      20MCA24 (SE)
                    </option>
                    <option value="20MCA251" data-credit="3">
                      20MCA251 (CS)
                    </option>

                    <option value="20MCA262" data-credit="3">
                      20MCA262 (AI)
                    </option>

                    <option value="20MCA263" data-credit="3">
                      20MCA263 (MAD)
                    </option>
                    <option value="20MCA255" data-credit="3">
                      20MCA255 (OT)
                    </option>

                    <option value="20MCA27" data-credit="2">
                      20MCA27 (DBMS LAB)
                    </option>
                    <option value="20MCA28" data-credit="2">
                      20MCA28 (JAVA LAB)
                    </option>

                    <option value="20MCA29" data-credit="2">
                      20MCA29 (WEB LAB)
                    </option>
                  </select>
                  <div className="error-container h-10 w-35">
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 sm:grid-cols-6 lg:grid-cols-6 gap-4 second_field">
                ...
                <div className="mb-6">
                  <input
                    ...
                    name="internal9"
                    id="internal9"
                    autoComplete="internal9"
                    value={internal9}
                    onChange={handleInternalChange9}
                    className="w-full px-3 py-2 text-xs lg:text-xl placeholder-gray-400 border rounded-lg focus:outline-none focus:shadow-outline-blue focus:border-blue-300"
                    placeholder="0"
                  />
                  <div className="error-container h-10 w-35">
                ...
                <div className="mb-6">
                  <input
                    ...
                    name="external9"
                    id="external9"
                    autoComplete="external9"
                    value={external9}
                    onChange={handleExternalChange9}
                    className="w-full px-3 text-xs lg:text-xl py-2 placeholder-gray-400 border rounded-lg focus:outline-none focus:shadow-outline-blue focus:border-blue-300"
                    placeholder="0"
                  />
                  <div className="error-container h-10 w-35">
